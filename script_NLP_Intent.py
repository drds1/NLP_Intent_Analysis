...
class lstm_fit():
    ...
    def match_word_embeddings(self):
        ...
        for i, word in self.idx_word.items():
            print(i, word, embeddings_words.get(word, None))
            # Look up the word embedding
            vector = embeddings_words.get(word, None)
            # Record in matrix
...
ypred_proba = model_lstm.predict(ytrain)
ypred_class = [np.argmax(yp) for yp in ypred_proba]
ypred_label = [labels[yp] for yp in ypred_class]
